...
import SecondHandList from '../components/sneakers/secondhand-list/SecondHandList'
...
function SneakersPage() {
  ...
  useEffect(() => {
    sneakersService.list()
      .then((sneakers) => {
        const notNewSneakers = sneakers.filter(sneaker => sneaker.new === false)
        setSneakers(notNewSneakers)
      })
  ...
  return (
    <div className='d-flex'>
      ...
      <div>
        ...
        <SecondHandList sneakersFiltered={sneakersFiltered} />
      </div>
    </div>
  )
}
